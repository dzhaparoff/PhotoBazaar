class Admin::Api::PhotosController < Admin::AdminController
	...
	def create
		...
		if !File.exist?('public'+params['local_image_url'])
	...
	def destroy
		...
		private
		  def photo_params
		    params.require(:photo).permit(:name, :thumbnails, :photographer_id, :description, :camera_id, :category_id, :photo_id,:lens, :focal_length, :iso, :shutter_speed, :aperture, :times_viewed, :rating, :location, :latitude, :longitude, :taken_at, :width, :height, :votes_count, :favorites_count, :nsfw, :highest_rating, :license_type, :original_url, :local_image_url, :base_color, :base_color_name, :colors)
		  end
